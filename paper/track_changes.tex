...
%DIF DEL paper_orig_submission.tex   Wed Mar  6 09:13:03 2019
%DIF ADD paper.tex                   Wed Mar  6 11:24:28 2019
...
\begin{flushleft}
{\Large
\textbf\newline{Single-cell virus sequencing of influenza infections that trigger innate immunity} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
Here we \DIFaddbegin \DIFadd{develop a new method to }\DIFaddend assess how the genetic variation inherent in viral populations contributes to this heterogeneity.
We do this by \DIFdelbegin \DIFdel{developing a new approach to determine }\DIFdelend \DIFaddbegin \DIFadd{determining }\DIFaddend both the transcriptome and full-length sequences of all viral genes in single influenza-infected cells.
Infections that activate an innate-immune response in single cells are \DIFdelbegin \DIFdel{associated with viral }\DIFdelend \DIFaddbegin \DIFadd{often initiated by virions with }\DIFaddend defects that include amino-acid mutations, internal deletions, and failure to express key genes.
...
Our work shows that the genetic variation present in influenza virus populations \DIFdelbegin \DIFdel{substantially }\DIFdelend contributes to but does not fully explain the heterogeneity in infection outcome and immune activation in single infected cells.
...
But so far, it has been impossible to fully characterize the genetic properties of the virions infecting any given cell, since the conventional techniques of flow cytometry and single-cell RNA-seq only detect if a protein or transcript is present---not its sequence.
Here we develop a new approach that uses long-read PacBio sequencing to determine the full sequences of the virions infecting individual cells.
Most cells are infected by virions that have a mutation or some other defect.
This viral genetic variation explains a substantial amount of the cell-to-cell variability in how much viral mRNA is produced in infected cells, and \DIFdelbegin \DIFdel{helps determine whether a cell activates }\DIFdelend \DIFaddbegin \DIFadd{some viral mutations make a cell more likely to activate }\DIFaddend an innate-immune response.
However, our results also show that viral genetic defects do not completely explain cell-to-cell variation in the outcome of infection.
Overall, this study provides the first complete picture of how viral mutations affect the outcome of infection in single cells.
...
Because influenza has a high mutation rate, \DIFdelbegin \DIFdel{even if the }\emph{\DIFdel{consensus}} %DIFAUXCMD
\DIFdel{sequence of a viral population is ``wild-type,'' individual virions will often have }\DIFdelend \DIFaddbegin \DIFadd{individual virions often have genetic }\DIFaddend defects~\cite{parvin1986measurement, suarez1992heterogeneity, suarez1994estimation, bloom2014experimentally, pauly2017novel}.
Indeed, many studies have identified mutations that increase IFN induction when engineered into a viral population~\cite{velthuis2018mini, du2018genome, killip2017single, perez2014unbiased}, and viral stocks that are rich in internal deletions in the polymerase genes induce more IFN~\cite{baum2010preference, tapia2013defective, boergeling2015evidence, dimmock2015cloned}.
...
Flow cytometry and fluorescent reporters only measure protein levels~\cite{brooke2013most, guo2017single}, and current single-cell transcriptomic techniques primarily measure abundance of transcripts and provide only fragmentary information on their sequences~\cite{russell2018extreme, zanini2018single, zanini2018virus, steuerman2018dissection, saikia2018simultaneous, oneal2018west}.
None of these techniques reliably reveal if the virion infecting a specific cell has some idiosyncratic mutation\DIFdelbegin \DIFdel{, despite the fact that such mutations are common even if cells are infected with a nominally ``wild-type'' stock of virus}\DIFdelend .

Here we develop a new approach \DIFdelbegin \DIFdel{to }\DIFdelend measure both the full transcriptome and sequences of all viral genes in single influenza-infected cells.
To do this, we first enrich for rare IFN+ cells, and then perform both standard Illumina-based \DIFdelbegin \DIFdel{single-cell }\DIFdelend transcriptomics and full-length PacBio sequencing of viral genes \DIFaddbegin \DIFadd{from single cells}\DIFaddend .
We obtain transcriptomes and sequences of all expressed viral genes in 150 infected cells, 40 of which express IFN.
\DIFdelbegin \DIFdel{Even though we used a ``wild-type'' stock of virus, two-thirds of }\DIFdelend \DIFaddbegin \DIFadd{Two-thirds of }\DIFaddend cells are infected by virions that have a mutation or defect in gene expression.
This viral diversity is a major contributor to cell-to-cell heterogeneity, with cells infected by \DIFdelbegin \DIFdel{truly wild-type }\DIFdelend \DIFaddbegin \DIFadd{unmutated }\DIFaddend virions having a tighter distribution of viral transcriptional burden.
\DIFdelbegin \DIFdel{Viral defects are especially common in IFN+ cells, and we identify at least four types of defects that cause increased }\DIFdelend \DIFaddbegin \DIFadd{We identify several types of viral defects that increase }\DIFaddend IFN induction.
However, viral genetic variation does not fully explain the heterogeneity, \DIFdelbegin \DIFdel{as even wild-type }\DIFdelend \DIFaddbegin \DIFadd{and even unmutated }\DIFaddend virions sometimes induce IFN.
...
\subsection*{A system to identify and enrich \DIFdelbegin \DIFdel{for }\DIFdelend rare IFN+ cells}
...
To \DIFdelbegin \DIFdel{efficiently }\DIFdelend identify and enrich rare IFN+ cells, we created A549 cells that carried IFN reporters consisting of a type I (\textit{IFNB1}) or type III (\textit{IFNL1}) promoter driving expression of a cell-surface protein (LNGFR$\Delta$C~\cite{bonini1997hsv,ruggieri1997cell}) followed by a fluorescent protein (\FIG{IFNrare}A).
...
The reporters were efficiently activated by \DIFaddbegin \DIFadd{infection with saturating amounts of }\DIFaddend a strain of Sendai virus~\cite{strahle2006sendai} that potently induces IFN (\SUPPFIG{reportervalidation}A), and activation of the type I and type III IFN reporters was highly correlated in our cells (\SUPPFIG{reportervalidation}B; further validated by the single-cell transcriptomics below).
Therefore, for the rest of this paper, we use ``IFN expression'' to refer to combined expression of type I and III IFNs. 

We generated a stock of \DIFdelbegin \DIFdel{``wild-type'' }\DIFdelend A/WSN/1933 (H1N1) influenza (hereafter referred to as ``WSN''), and found that it activated the IFN reporter in $\sim$0.5\% of infected cells  (\FIG{IFNrare}B), a frequency roughly comparable to that in our prior single-cell transcriptomics of influenza-infected cells.
...
From 12 to 13 hours post-infection, we used MACS to enrich cells that \DIFdelbegin \DIFdel{had }\DIFdelend activated the IFN reporter (\SUPPFIG{MACS}).
...
To remove some of these multiplets \DIFaddbegin \DIFadd{along with low-quality droplets}\DIFaddend , we filtered transcriptomes with unusually high or low numbers of cellular transcripts \DIFdelbegin \DIFdel{(}%DIFDELCMD < \FIG{transcriptomics}%%%
\DIFdel{B), retaining the }\DIFdelend \DIFaddbegin \DIFadd{as is commonly done in analysis of single-cell RNA-seq data~\mbox{%DIFAUXCMD
\cite{haque2017practical}}\hspace{0pt}%DIFAUXCMD
.
After this filtering, we retained }\DIFaddend 1,490 human cells \DIFdelbegin \DIFdel{that passed this filtering.
}\DIFdelend \DIFaddbegin \DIFadd{for further analysis (}\FIG{transcriptomics}\DIFadd{B)
}\DIFaddend 

To identify infected cells, we examined the fraction of each transcriptome derived from virus (\FIG{transcriptomics}C).
As expected, only a small fraction ($\sim$0.7\%) of transcripts in the uninfected canine cells were \DIFdelbegin \DIFdel{from influenza}\DIFdelend \DIFaddbegin \DIFadd{viral}\DIFaddend ; this low-level background is likely from lysed cells that release ambient viral mRNA that is captured in droplets during reverse-transcription.
...
We \DIFdelbegin \DIFdel{then }\DIFdelend called the presence or absence of each viral gene in each infected cell, again using a Poisson model parameterized by background fractions estimated from uninfected canine cells.
...
\FIG{transcriptomics}D (top panel) shows that \DIFdelbegin \DIFdel{most (}\DIFdelend 162 of 290 \DIFdelbegin \DIFdel{) }\DIFdelend infected cells express all eight genes \DIFdelbegin \DIFdel{, although a substantial minority fail to express one or more genes }\DIFdelend (see \SUPPFIG{frac_has_gene} for frequencies \DIFdelbegin \DIFdel{at which individual genesare present}\DIFdelend \DIFaddbegin \DIFadd{for individual genes}\DIFaddend ).
...
The amount of viral mRNA \DIFdelbegin \DIFdel{tended to be }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend lower in cells that failed to express viral genes (\FIG{transcriptomics}D, bottom panel).
...
Over 20\% of infected cells were IFN+ given the heuristic thresholds in \FIG{transcriptomics}G, indicating that the MACS successfully enriched IFN+ cells far beyond their initial frequency\DIFdelbegin \DIFdel{of $\sim$0.5\% (}%DIFDELCMD < \FIG{IFNrare}%%%
\DIFdel{B) among infected cells}\DIFdelend .
...
These results are consistent with the established knowledge that IFN is expressed only in cells that directly detect infection, but that ISGs are also expressed via paracrine signaling in cells that do not \DIFdelbegin \DIFdel{themselves }\DIFdelend \DIFaddbegin \DIFadd{directly }\DIFaddend detect infection~\cite{stetson2006type,honda2006type}.
...
\begin{fullwidth}
{\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=0.92\textheight]{figures/single_cell_figures/p_genotypes.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.9\textheight]{figures/single_cell_figures/p_genotypes.pdf}
\DIFaddendFL }
\caption{
{\bf Viral genotypes and infection outcomes in single cells.}
\DIFdelbeginFL \DIFdelFL{Arrows }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Green and orange boxes at the left show the percent of all mRNA in that cell derived from virus and the percent of all cellular mRNA derived from IFN, respectively.
The second box is framed in orange for cells classified as IFN+ in }\FIG{transcriptomics}\DIFaddFL{G.
Blue arrows }\DIFaddendFL indicate \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL presence of a viral gene from one (light blue) or both \DIFaddbeginFL \DIFaddFL{(dark blue) }\DIFaddendFL viral barcode variants\DIFdelbeginFL \DIFdelFL{(}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; a }\DIFaddendFL dark blue \DIFdelbeginFL \DIFdelFL{)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{arrow therefore means that a cell was co-infected}\DIFaddendFL .
Circles and boxes \DIFaddbeginFL \DIFaddFL{on the arrows }\DIFaddendFL indicate mutations or indels as described in the legend \DIFdelbeginFL \DIFdelFL{, with }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{at right.
The }\DIFaddendFL circle areas and box heights \DIFaddbeginFL \DIFaddFL{are }\DIFaddendFL proportional to the fraction of CCSs with that mutation.
...
\DIFdelbeginFL \DIFdelFL{Green boxes at left show the percent of all mRNA in that cell derived from virus.
Orange boxes show the percent of cellular mRNA derived from IFN}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{For instance}\DIFaddendFL , \DIFaddbeginFL \textit{\DIFaddFL{cell 5}} \DIFaddFL{was co-infected by a virion }\DIFaddendFL with \DIFdelbeginFL \DIFdelFL{orange box frames indicating cells classified as IFN+ in }%DIFDELCMD < \FIG{transcriptomics}%%%
\DIFdelFL{G}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{one wild-type and one internally deleted copy of PB1}\DIFaddendFL .
}
\label{fig:genotypes}
...
\DIFdelbegin \DIFdel{We called a mutation in a viral gene if it was found in }\DIFdelend \DIFaddbegin \DIFadd{After calling the presence / absence of each viral gene in each cell using the transcriptomic data as described in the previous section, we called mutations if they were found in at }\DIFaddend least two CCSs originating from different mRNAs (unique UMIs) and at least 30\% of all CCSs for that gene in that cell.
...
For instance, the cell with the highest viral burden (\textit{cell 1} in \FIG{genotypes}, which has 65\% of its mRNA from virus) was infected by a virion that expressed \DIFdelbegin \DIFdel{wild-type }\DIFdelend \DIFaddbegin \DIFadd{unmutated }\DIFaddend copies all eight genes and did not induce detectable IFN.
But 12 of the other 13 cells with at least 50\% of their mRNA from virus were infected by virions that had a mutation or failed to express \DIFdelbegin \DIFdel{at least one }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend gene, and five of these cells expressed IFN.
\DIFaddbegin \DIFadd{As expected, all cells infected by virions that failed to express a component of the viral polymerase complex (PB2, PB1, PA, or NP) expressed low amounts of viral mRNA since they are limited to primary transcription off the incoming proteins (e.g., }\textit{\DIFadd{cell 132}} \DIFadd{and }\textit{\DIFadd{cell 143}}\DIFadd{).
}\DIFaddend The two cells that expressed the most IFN (\textit{cell 13} and \textit{cell 123}) both lacked the viral NS gene \DIFdelbegin \DIFdel{, and many }\DIFdelend \DIFaddbegin \DIFadd{that encodes the virus's primary IFN antagonist~\mbox{%DIFAUXCMD
\cite{garcia1998influenza, hale2008multifunctional}}\hspace{0pt}%DIFAUXCMD
.
Many }\DIFaddend other IFN+ cells had different defects such as large internal deletions (e.g., \textit{cell 5} and \textit{cell 89}) or amino-acid mutations (e.g., \textit{cell 9}, \textit{cell 28}, and many others).
\DIFdelbegin \DIFdel{However, }\DIFdelend \DIFaddbegin 

\DIFadd{However, }\FIG{genotypes} \DIFadd{also reveals stochasticity that is independent of viral genotype.
This stochasticity sometimes acts to the detriment of the virus, and sometimes to the detriment of the cell.
As an example of the former case, }\DIFaddend expressing unmutated copies of all eight genes did not guarantee a favorable outcome for the virus: for instance, the \DIFdelbegin \DIFdel{wild-type }\DIFdelend \DIFaddbegin \DIFadd{unmutated }\DIFaddend virion that infected \textit{cell 139} only managed to express viral mRNA to 6\% of the total transcriptome, and the \DIFdelbegin \DIFdel{wild-type }\DIFdelend \DIFaddbegin \DIFadd{unmutated }\DIFaddend virion that infected \textit{cell 105} still induced IFN.
\DIFaddbegin \DIFadd{But in other cases, the stochasticity allows a clearly defective virus to still escape immune recognition.
For instance, there are a number of cells (e.g., }\textit{\DIFadd{cell 62}} \DIFadd{and }\textit{\DIFadd{cell 78}}\DIFadd{) that do not activate IFN despite being infected by virions that fail to express NS.
}\DIFaddend 
...
\caption{
{\bf Viral features associated with heterogeneity in infection outcome among cells for which we \DIFdelbeginFL \DIFdelFL{sequenced all expressed }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{determined }\DIFaddendFL viral \DIFdelbeginFL \DIFdelFL{genes}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{genotypes}\DIFaddendFL .}
{\bf (A)} 
Percent of all mRNA derived from virus, faceted by whether cells express unmutated copies of all eight genes.
Cells infected by fully \DIFdelbeginFL \DIFdelFL{wild-type virus }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{unmutated virions }\DIFaddendFL exhibit less heterogeneity in viral burden as quantified by the Gini index (95\% confidence intervals are indicated).
{\bf (B)}
...
\DIFdelbegin \DIFdel{Due to the low number of cells and large number of hypotheses in }%DIFDELCMD < \FIG{mutations}%%%
\DIFdel{C, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend only trend that remained significant at a false discovery rate \DIFaddbegin \DIFadd{(FDR) }\DIFaddend of 10\% was absence of NS.
\DIFaddbegin \DIFadd{This lack of statistical significance after FDR correction could be due to the relatively modest number of fully sequenced infected cells (just 150).
...
\subsection*{Validation that viral defects in single IFN+ cells often \DIFdelbegin \DIFdel{cause increased }\DIFdelend \DIFaddbegin \DIFadd{increase }\DIFaddend IFN induction}
...
The single-cell data also showed that amino-acid substitutions in the PB1 and NS genes were enriched in IFN+ cells (\FIG{genotypes}, \FIG{mutations}C), so we created mutant viruses with \DIFdelbegin \DIFdel{many of the amino-acid }\DIFdelend \DIFaddbegin \DIFadd{some of the }\DIFaddend substitutions found in these genes among IFN+ cells\DIFdelbegin \DIFdel{.
Overall, we created six such point-mutant viruses}\DIFdelend :
PB1-D27N, PB1-G206S, PB1-K279R, PB1-T677A, NS1-A122V, and NS2-E47G.

Finally, prior work has suggested that virions \DIFdelbegin \DIFdel{carrying }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend internal deletions in the polymerase genes can induce higher levels of IFN~\cite{baum2010preference, tapia2013defective, boergeling2015evidence, dimmock2015cloned}.
Although such deletions are not significantly enriched among IFN+ cells in our single-cell data (\FIG{mutations}C), there is \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a co-infected }\DIFaddend IFN+ cell \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{where one viral variant }\DIFaddend has a deletion in PB1 spanning nucleotides 385 to 2163 (\textit{cell 5} in \FIG{genotypes}).
...
\FIG{validation} shows that five of the eight mutant viral stocks induced IFN more frequently than \DIFdelbegin \DIFdel{wild type}\DIFdelend \DIFaddbegin \DIFadd{a wild-type viral stock}\DIFaddend .
...
\DIFaddbegin \DIFadd{However, IFN induction remains stochastic even for the most potently IFN-inducing viral mutants.
}\FIG{validation} \DIFadd{shows flow cytometry data (see also }\SUPPFIG{validationflow}\DIFadd{), which is itself a single-cell measurement, albeit one that does not report the viral genotype.
...
But IFN induction is stochastic at the single-cell level even for infections that share the same defect,such as absence of NS (e.g., compare }\textit{\DIFadd{cell 62}} \DIFadd{and }\textit{\DIFadd{cell 69}} \DIFadd{in }\FIG{genotypes}\DIFadd{).
...
Our method builds on the observation \DIFdelbegin \DIFdel{by }\DIFdelend that fragmentary viral genetic information can be obtained by more standard single-cell transcriptomic techniques~\cite{saikia2018simultaneous, zanini2018virus}.
...
\DIFdelbegin \DIFdel{It turns out that these }\DIFdelend \DIFaddbegin \DIFadd{These }\DIFaddend viral defects substantially contribute to the heterogeneity observed in prior single-cell studies of influenza~\cite{russell2018extreme, steuerman2018dissection, heldt2015single, sjaastad2018distinct}, including helping determine if a cell triggers an IFN response.
We identified four types of viral defects in single IFN+ cells that we validated to increase IFN induction. 
Two types of defects---absence of the NS gene and amino-acid \DIFdelbegin \DIFdel{mutation }\DIFdelend \DIFaddbegin \DIFadd{mutations }\DIFaddend to the NS1 protein---presumably impair NS1's well-known ability to antagonize innate-immune pathways~\cite{garcia1998influenza, hale2008multifunctional}.
...
Finally, we found an internal deletion in the PB1 gene that enhances IFN induction, consistent with \DIFdelbegin \DIFdel{a large body of }\DIFdelend prior work showing that such deletions are immunostimulatory~\cite{baum2010preference, tapia2013defective, boergeling2015evidence, dimmock2015cloned}.
...
We find substantial breadth in viral transcriptional burden and occasional IFN induction even among cells infected with \DIFdelbegin \DIFdel{fully wild-type }\DIFdelend \DIFaddbegin \DIFadd{unmutated }\DIFaddend virions.
Additionally, \DIFdelbegin \DIFdel{none of the viral defects in IFN + cells induce IFN deterministically in our validation experiments---and the apparently }\DIFdelend \DIFaddbegin \DIFadd{no viral defect induces IFN deterministically---and the }\DIFaddend most immunostimulatory defect (absence of NS) also occurs in multiple IFN- cells in our single-cell dataset.
...
It is therefore plausible that \DIFdelbegin \DIFdel{genetic variation in the founding viral population }\DIFdelend \DIFaddbegin \DIFadd{early heterogeneity }\DIFaddend could have a large effect on downstream events.
...
This process physically separates disparate template molecules, preventing strand exchange and allowing each molecule to be amplified to exhaustion of its droplet's reagents without competing with the broader pool of PCR-amenable molecules~\cite{boers2015micelle}.
...
%DIF < ABR was supported by a postdoctoral fellowship from the Damon Runyon Cancer Research Foundation.
%DIF > ABR was supported by a postdoctoral fellowship from the Damon Runyon Cancer Research Foundation (DRG-2227-15).
...
\caption{
{\bf Re-analysis of a published single-cell transcriptomic study~\cite{steuerman2018dissection} of influenza-infected mice shows that only 5 of 1220 virus-infected cells express \emph{detectable} type I or type III IFN transcripts \textit{in vivo}.}
Specifically, we downloaded the data from \cite{steuerman2018dissection} and identified influenza-infected cells using criteria similar to those described in \cite{steuerman2018dissection}.
...
As shown in the left-most panel, the sequencing depth of \cite{steuerman2018dissection} was quite low, with only $\sim$1,500 mRNA counts per cell on average (this is 10 to 15-fold lower than the sequencing depth in the current study).
...
Nonetheless, there is detectable expression of key ISGs---which are induced by autocrine and paracrine signaling~\cite{stetson2006type,honda2006type}---in the majority of infected cells (the middle panel shows the total counts of IFIT1, ISG15, CCL5, and Mx1).
However, only 5 of 1220 cells express any detectable type I (IFN-$\alpha$ and IFN-$\beta$) or type III (IFN-$\lambda$) mRNAs, which are only induced by direct cellular detection of viral infection~\cite{stetson2006type,honda2006type}.
...
\caption{
{\bf Validation of reporter cell lines (\FIG{IFNrare}A) to identify IFN+ cells.}
{\bf (A)}
To validate the IFN reporter cell lines, they were infected at high MOI with the Cantell strain of Sendai virus, which strongly activates IFN expression~\cite{strahle2006sendai}.
...
\caption{
{\bf Flow cytometry data for \FIG{IFNrare}B.}
The A549 cells with the \textit{IFNL1} reporter driving LNGFR$\Delta$C-ZsGreen were not infected, infected with saturating amounts of the Cantell strain of Sendai virus~\cite{strahle2006sendai}, or infected the same stock of influenza virus used in the single-cell experiment at a target MOI of 0.3.
...
\caption{
{\bf Example MACS enrichments of IFN+ influenza-infected cells.}
...
\caption{
{\bf Expression of ISGs in single infected and uninfected cells.}
...
This is probably because paracrine signaling can induce ISG expression in cells that are not themselves expressing IFN~\cite{stetson2006type,honda2006type}.
...
\caption{
{\bf The number of PacBio CCSs that passed quality-control steps and aligned to an influenza virus gene.}
...
From the frequencies of these different forms, we estimate~\cite{bloom2018estimating} that 5.7\% of molecules are chimeric due to PCR strand exchange.
...
\caption{
{\bf Strategy for detecting strand exchange during sequencing of full-length viral genes.}
...
Note that since the rate of homologous recombination in influenza virus in negligible~\cite{boni2008homologous}, such mixed-barcode molecules are \emph{not} expected to be generated naturally during co-infection.
}
...
These infections were performed in the presence of absence of 50 $\mu$g/ml cycloheximide, which blocks protein synthesis and hence secondary transcription by newly synthesized viral proteins~\cite{killip2014activation}.
...
\begin{suppfile}
\caption{{\bf Sequences of the IFN reporters in \FIG{IFNrare}A.}
%See \url{https://github.com/jbloomlab/IFNsorted_flu_single_cell/tree/master/paper/figures/IFN_stochastic/IFN_reporter/plasmids}.
}
...
\begin{suppfile}
\caption{
{\bf Genbank files giving sequences of the wild-type and synonymously barcoded viruses}.
%See \url{https://github.com/jbloomlab/IFNsorted_flu_single_cell/blob/master/data/flu_sequences/flu-wsn.gb} and \url{https://github.com/jbloomlab/IFNsorted_flu_single_cell/blob/master/data/flu_sequences/flu-wsn-double-syn.gb}.
}
...
\begin{suppfile}
\caption{
{\bf Genbank plasmid maps for the mutant genes cloned into the pHW* bi-directional reverse genetics plasmid~\cite{hoffmann2000dna}.}
%See \url{https://github.com/jbloomlab/IFNsorted_flu_single_cell/tree/master/paper/figures/FluVariantPlasmidMaps}
}
