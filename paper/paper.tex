...
The foregoing transcriptomic analyses identify which viral genes are expressed in each cell, but they do not provide information on the sequences of these genes.
...
We therefore used the approach schematized in \FIG{workflow}D to determine the full-length sequences of the viral genes expressed in the majority of infected cells.
...
We called a mutation as present in a particular cell if it was found in least two CCSs originating from different mRNAs (unique UMIs) in that cell, and was also present in at least 30\% of all CCSs for that gene in that cell.
For cells co-infected with both viral barcode variants, we called mutations separately for each viral variant.
This strategy reliably identifies mutations in the virions that initiate infection of cells infected with at most one virion of each viral barcode variant (estimated to be $\sim$75\% of infected cells), as well as identifying high-abundance mutations in cells co-infected with multiple virions of the same viral barcode.
...
\caption{
Viral genetic variation associated with heterogeneity in infection outcome among cells for which we sequenced all expressed viral genes.
{\bf (A)} 
...
To more systematically assess how viral genetic variation associated with infection outcome, we divided the 150 cells in \FIG{genotypes} into those that expressed unmutated copies of all eight genes (disregarding synonymous mutations) and those that did not (due to gene absence, amino-acid substitution, insertion, or deletion).
...
Viral genetic variation also associated with IFN induction, since cells infected by full wildtype virions less IFN than cells infected by incomplete or mutated virions (\FIG{mutations}B).
...
The points are colored orange if the mutant virus stock induces IFN more frequently than the wildtype viral stock (t-test, $P < 0.01$), and blue if the frequency of IFN induction is not significantly different than wildtype at this cutoff.
{\bf (B)}
...
See \FIGSUPP[validation]{delflow} for detailed flow cytometry data, and \FIGSUPP[validation]{delqPCR} for details of how we validated that the infections were performed with a similar doses of each viral mutant.
The experiments in the two panels were performed on different days, and so exact numerical values can be reliably compared within each panel but not between panels.
}
...
To rescue these viruses, we used 293t cells and MDCK-SIAT1 cells transduced to constituitively express PB1 as previously described~\citep{bloom2010permissive}. 
