...
We next used PacBio sequencing (\FIG{workflow}D, \SUPPFILE{primer_sequences}) to determine the full sequences of the viral genes in single infected cells.
...
The 150 cells for which we called the full viral genotypes are shown in \FIG{genotypes} (see also \SUPPFILE{genotypes}).
...
\FIG{mutations}A shows that the 49 cells infected by full unmutated virions had a significantly tighter distribution of the amount of viral mRNA per cell than the other 101 cells as quantified by the Gini index~\cite{gini1921measurement} (see also \SUPPFILE{mutations}).
...
The raw data are in \SUPPFILE{genotypes} and \SUPPFILE{mutations}.
