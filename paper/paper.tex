...
\begin{abstract}
The outcome of viral infection is extremely heterogeneous at the cellular level, and infected cells only sometimes activate innate immunity.  
...
As the virus spreads, some infected cells trigger innate immunity and begin producing interferon~\citep[IFN;][]{iwasaki2014innate}.
...
If innate immunity is activated sufficiently rapidly, it can reduce viral replication and disease severity~\citep{solov1969results,treanor1987intranasally,beilharz2007protection,kugel2009intranasal,steel2010transmission}.

Unfortunately for the host, influenza infection only rarely triggers IFN production by individual infected cells~\citep{kallfass2013visualizing, killip2017single}.
...
Because viral replication and the IFN response are both feed-forward processes, early cell-to-cell heterogeneity could have significant downstream consequences for the race between virus and immune system---especially since real influenza infections are typically initiated by just a few virions entering a few cells~\citep{mccrone2018stochastic, xue2018reconciling, varble2014influenza}.

It remains unclear what determines if an infected cell triggers an innate-immune response.
Two possible contributors are pure stochasticity and pre-existing variation in cellular state.
For instance, only some cells induce IFN even upon homogeneous treatment with synthetic innate-immune ligands~\citep{shalek2013single, shalek2014single, wimmers2018single}, and there is evidence that the frequency of IFN induction depends on a cell's pre-existing chromatin state~\citep{bhushal2017cell}.
But for influenza, a third possible contributor also looms large: viral genetic diversity.
Specifically, because influenza has a high mutation rate, even if the \emph{consensus} sequence of a viral population is ``wildtype,'' individual virions will often have genetic defects~\citep{parvin1986measurement, suarez1992heterogeneity, suarez1994estimation, bloom2014experimentally, pauly2017novel}.
Indeed, many studies have identified mutations that increase IFN induction when engineered into a viral population~\citep{velthuis2018mini, du2018genome, killip2017single, perez2014unbiased}, and more IFN is induced by viral stocks that contain defective virions with internal deletions in the polymerase genes~\citep{baum2010preference, tapia2013defective, boergeling2015evidence, dimmock2015cloned}.

However, existing single-cell techniques are inadequate to determine how viral genetic diversity actually contributes to cell-to-cell heterogeneity during infection, since they do not systematically determine the full sequences of the virions that infect individual cells.
Flow cytometry and fluorescent reporters only measure expression of viral proteins~\citep{brooke2013most, guo2017single}, and current single-cell transcriptomic techniques primarily measure abundance of viral RNAs~\citep{russell2018extreme, zanini2018single, zanini2018virus, steuerman2018dissection, saikia2018simultaneous}.
None of these techniques reliably reveal if the virion infecting a specific cell has some idiosyncratic mutation, despite the fact that such mutations will be common regardless of whether the cells are infected with a nominally ``wildtype'' or ``mutant'' stock of virus.

Here we develop a new approach to measure both the full transcriptome and the sequences of all viral genes in single influenza-infected cells that do and do not activate innate immunity.
...
We obtain the transcriptomes and full sequences of all expressed viral genes in 150 virus-infected cells, 49 of which express IFN.
Even though we used a nominally ``wildtype'' stock of virus, two-thirds of the cells are infected by virions that have a mutation or defect in gene expression.
This viral diversity is a major contributor to cell-to-cell heterogeneity, with cells infected by truly wildtype virions having a tighter distribution of viral transcriptional burden.
Viral defects are especially common in IFN+ cells, and we identify at least four defects that are causally associated with IFN induction: amino-acid substitutions in the viral polymerase (PB1), absence of the NS gene, amino-acid substitutions in NS1, and internal deletions in polymerase genes.
However, viral genetic variation does not fully explain the cell-to-cell heterogeneity, as even truly wildtype virions sometimes induce IFN, and no viral mutations that we identify deterministically induce IFN.

\clearpage
\section{Results}

\subsection{A system to identify and enrich for rare IFN+ positive cells}
Influenza infection only rarely triggers expression of the anti-viral IFNs~\citep[type I and type III IFNs;][]{kotenko2017contribution} at the single-cell level~\citep{killip2017single}.
...
Therefore, for the rest of this paper, we will use ``IFN expression'' to refer to the combined expression of type I and III IFNs. 
...
Such measurements of transcriptomic abundance do not reveal if the virions that infect individual cells have features (e.g., mutations) that contribute to heterogeneity in infection outcome. 
...
We therefore developed the approach in \FIG{workflow} to determine both the entire transcriptome \emph{and} the full sequences of the infecting virions in single cells.
First, we generated a stock of virus that consisted of a mix of two variants of the WSN strain of influenza: the wildtype virus and a ``synonymously barcoded'' variant that contained two engineered synonymous mutations near each termini of each gene (\FIGDATA[workflow]{virus_seqs}).
...
We next used this viral stock to infect A549 cells carrying an IFN reporter (\FIG{workflow}A) at a dose that led to detectable viral transcription in about a quarter of the cells.
...
To ensure the presence of IFN- cells after the MACS enrichment, we added back non-enriched cells to $\sim$10\% of the total.
We also added uninfected canine cells to $\sim$5\% of the total as an internal control for multiplets and to estimate the background amount of viral mRNA detected in truly uninfected cells.
...
We classified the other cells as uninfected, although it is possible that some of them were infected with virions that produced too little mRNA to be detected in our experiment.
The distribution of the amount of viral mRNA across infected cells is shown in the inset in \FIG{transcriptomics}B.
As in our prior work~\citep{russell2018extreme}, the amount of viral mRNA is extremely heterogeneous: many infected cells have only a few percent of their mRNA derived from virus, but viral mRNA comprises over half the transcriptome of a few cells.
...
For instance, the cell with the highest viral burden (\textit{cell 1} in \FIG{genotypes}, which has 65\% of its mRNA from virus) was infected by a virion that expressed wildtype copies all eight genes and did not induce detectable IFN.
...
The limit of detection of 0.05\% is indicated with a dashed green line, and the median value for wildtype is indicated with a dashed blue line.
Points are colored orange if the mutant virus stock induces IFN more frequently than the wildtype viral stock (one-sided t-test, $P < 0.01$), and blue otherwise.
{\bf (B)}
...
We therefore created a virus carrying this deletion that we were able to propagate in cells constitutively expressing the PB1 protein.

We next tested the rate of IFN induction by each viral stock using the reporter cell lines.
As shown in \FIG{validation}, five of the eight mutant viruses induced IFN more frequently than wildtype.
The strongest IFN induction was by the NS1stop virus, but the PB1 internal deletion and three of the six point-mutant viruses (PB1-D27N, PB1-T677A, and NS1-A122V) also induced significantly more IFN than wildtype.
...
Therefore, although the increased IFN induction by most viral variant stocks in \FIG{validation} does indicate that these variants are causally associated with IFN, without single-cell virus-sequencing it is impossible to know how many of the IFN+ cells induced by each viral stock are infected by virions that also have other defects.
For instance, most of the 0.67\% of cells infected with the ``wildtype'' viral stock that turned IFN+ in \FIG{validation}A are probably infected by virions that have some type of genetic defect. 

\subsection{Distinct IFN-inducing characteristics of different viral variants}
...
{\bf (A)}
Flow cytometry shows that for viruses with defects in NS1, there is increased IFN induction among infected cells that express high levels of HA protein relative to those that express low levels of HA.
However, for wildtype virus and viruses with mutations in PB1, there is only a modest increase in IFN induction with increasing HA.
The y-axis shows the ratio of the percent of cells that are IFN+ in the highest HA-expression quartile relative to the lowest HA-expression quartile.
Points indicate four replicates, and lines indicate the mean.
See \FIGSUPP[mutantcomparison]{ha_vs_ifn} for details.
{\bf (B)}
Higher viral gene expression \emph{causes} more IFN induction for NS1-deficient infections, since blocking viral secondary transcription strongly decreases IFN levels in cells infected with NS1stop virus, but not cells infected with the virus carrying a deletion in PB1.
A549 cells were infected with both viral stocks at doses that induced equivalent IFN, and ribavirin was added to half the samples at a concentration (100 $\mu$M) sufficient to block secondary transcription~\citep{Vanderlinden:2016ec,Reuther:2015ef,Scholtissek:1976wg}.
...
\FIGSUPP[mutantcomparison]{cycloheximide} shows that similar results are obtained if secondary transcription is blocked with cycloheximide rather than ribavirin.
}
\label{fig:mutantcomparison}

\figsupp[Flow cytometry analysis showing that viral burden strongly correlates with IFN induction in NS1-deficient infections.]
{Re-analysis of the flow cytometry data in \FIGSUPP[validation]{SNPflow} indicates that a higher viral burden is associated with increased IFN-induction in NS1-deficient infections.
We re-analyzed all the infected cells (defining ``infected'' using the same HA-gating scheme in \FIGSUPP[validation]{SNPflow}) for the wildtype virus, the NS1stop virus, and the three amino-acid mutants with significantly increased IFN induction.
...
But for wildtype and the two PB1 mutants, there is no clear trend for higher viral burden to correlate with IFN induction.
...
{Preventing viral secondary transcription by addition of the cycloheximide also decreases the rate of IFN induction by the NS1stop virus relative to the PB1 deletion virus, corroborating the result in \FIG{mutantcomparison}B with a different inhibitor.
...
Therefore, we see an increase in IFN levels upon cycloheximide treatment for the uninfected cells and cells infected with the PB1 deletion virus, but a decrease in cells infected with the NS1stop virus.
...
As described in \FIGSUPP[validation]{cycloheximide}, A549 cells were infected at an MOI of 0.4, such that HA expression was equivalent between viral variants when secondary transcription was blocked, and some of the samples treated with 50 $\mu$g/ml of cycloheximide.
...
\jdbcomment{Section needs to be completed after remaining data on co-infections.}

\section{Discussion}

\jdbcomment{Section needs to be complete.
Add this:
Approach builds on observations by \citet{saikia2018simultaneous} and \citet{zanini2018virus} that fragmentary viral genetic information can be obtained using more standard single-cell techniques, and is similar the approach used by \cite{gupta2018single} to obtain full-length isoforms of some cellular genes in single cells.
}
