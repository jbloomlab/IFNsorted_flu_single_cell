...
\title{Combined single-cell transcriptomics and virus sequencing of influenza infections that trigger innate immunity}
...
We first analyzed the transcriptomic data.
...
As described previously~\citep{russell2018extreme}, the amount of viral mRNA is extremely heterogeneous: many infected cells have only a few percent of their mRNA derived from virus, but for a few cells viral mRNA comprises over half the transcriptome.
...
Using the synonymous barcodes near the 3' termini of transcripts from one of the two infecting viral variants, we determined that 38\% of cells were co-infected with wildtype and synonymously barcoded virions (\FIG{transcriptomics}F; cells called as co-infected if a binomial test rejected the null hypothesis that $\ge$95\% of viral mRNA is from one viral barcode variant).
From the numbers in \FIG{transcriptomics}F, we estimate~\citep{bloom2018estimating} that 63\% of infected cells are co-infected, implying that 25\% are co-infected with two virions with the same viral barcode (such co-infections cannot be directly identified from the transcriptomic data).
...
\subsection{The full genotypes of viruses in single IFN+ and IFN- cells}
...
The foregoing transcriptomic analyses identify which viral genes are expressed in each infected cell, but they do not provide information on the sequences of these genes.
This is a major shortcoming, since influenza's high mutation rate means that although the \emph{consensus} sequence of the viral stock used in our experiment corresponds to WSN influenza, the specific virions infecting individual cells will often carry mutations~\citep{parvin1986measurement, suarez1992heterogeneity, suarez1994estimation, bloom2014experimentally, pauly2017novel}.
We therefore used the approach schematized in \FIG{workflow}D to determine the full-length sequences of all viral genes expressed in the majority of infected cells.
...
Crucially, the fact that about half the infecting virions encoded genes that were tagged with synonymous viral barcodes at both termini enabled us to estimate the rate of PCR strand exchange among cDNAs that we amplified for PacBio sequencing (\FIGSUPP[genotypes]{StrandExchange}).
...
We called a mutation as present in a viral gene in a particular cell if it was found in least two CCSs originating from different mRNAs (unique UMIs) in that cell, and was also present in at least 30\% of all CCSs for that gene in that cell.
For cells co-infected with both viral barcode variants, we called mutations separately for each viral variant.
This strategy reliably identifies mutations in the virions that initiate infection of cells infected with at most one virion of each viral barcode variant, as well as identifying high-abundance mutations in cells co-infected with multiple virions of the same viral barcode.
The strategy will not identify mutations that arise within a cell after the first few rounds of viral genome replication, since such mutations will not reach 30\% frequency among viral transcripts in that cell.
Therefore, analogous to somatic variant calling in tumor sequencing~\citep{xu2014comparison, cibulskis2013sensitive}, there is an inherent limit to our detection threshold: we can identify viral mutations that are at high frequency within single infected cells, but not mutations that are present on just a small fraction of the viral transcripts in a cell. 

We were able to call the sequences of all expressed viral genes in the majority of infected cells (\FIGSUPP[genotypes]{ncells}).
We were most effective at calling full viral genotypes in cells that expressed high amounts of viral mRNA and were not co-infected by both viral barcode variants (\FIGSUPP[genotypes]{ncells}).
But despite this bias towards calling full viral genotypes from cells with these characteristics, we also called full genotypes for many cells that had low viral burden or were co-infected by both viral barcode variants.
...
The two cells that expressed the most IFN in our experiment (\textit{cell 13} and \textit{cell 123}) both lacked the viral NS segment, which encodes influenza virus's primary innate immune antagonist NS1.
Many other IFN+ cells had other defects, including large internal deletions (e.g., \textit{cell 5} and \textit{cell 89}) or amino-acid mutations (e.g., \textit{cell 9}, \textit{cell 28}, and many others).
However, \FIG{genotypes} also shows that expressing unmutated copies of all eight genes does not guarantee a favorable outcome for the virus: for instance, the apparently flawless virion that infected \textit{cell 139} still only managed to express viral mRNA to only 6\% of the total transcriptome, and the apparently flawless virion that infected \textit{cell 105} still induced IFN expression.

\subsection{Systematic examination of viral features associated with infection outcome}
...
\caption{
Viral genetic variation partially explains heterogeneity in infection outcome among cells for which we sequenced all expressed viral genes.
{\bf (A)} 
Percent of all mRNA derived from virus, faceted by whether cells express unmutated copies of all eight viral genes, or fail to express an unmutated copy of one or more genes.
...
\FIGSUPP[mutations]{allcells} shows that the trends remain largely the same if we include infected cells with incomplete viral sequence information. 
\FIGSUPP[mutations]{isg} shows that the trends remain largely the same if we categorize cells by ISG expression rather than IFN expression.
{\bf (D)}
...
To more systematically assess how viral genetic variation affected infection outcome, we divided the 150 cells in \FIG{genotypes} into those that expressed unmutated copies of all eight genes (disregarding synonymous mutations) and those that did not (due to gene absence, amino-acid substitution, insertion, or deletion).
...
Viral genetic variation also contributed to the heterogeneity in IFN induction, with cells infected by full wildtype virions less likely to express IFN than cells infected by incomplete or mutated virions (\FIG{mutations}B).
Furthermore, specific viral genetic defects were associated with IFN induction (\FIG{mutations}C).
...
Only the absence of NS remained associated with IFN induction at a false discovery rate of 10\% (\FIG{mutations}C), although a Bayesian-minded virologist might argue that controlling for all the hypotheses in \FIG{mutations}C is too stringent since the association of defects in certain genes (particularly NS and the polymerase genes) with IFN induction is more plausible \textit{a priori} given existing biological knowledge~\citep[e.g.,][]{killip2017single, velthuis2018mini, wu2014high}.
In any case, the modest number of cells with full viral genotype information precludes drawing strong conclusions from statistical analyses alone---but \FIG{genotypes} and \FIG{mutations}C do motivate compelling hypotheses about specific viral mutations that contribute to IFN induction, and for most of the rest of this paper we pursue these hypotheses using experimental follow-up. 

One other interesting trend emerges from analyzing the combined single-cell transcriptomic and viral genetic information.
...
The statistical analysis in \FIG{mutations}D is insufficient to determine if the higher viral transcriptional burden in IFN+ NS-deficient infections is a cause or a consequence of the IFN induction, but the experimental follow-up below will also shed more light on this question.

\subsection{Validation that multiple types of viral mutations increase IFN induction}
...
All viruses were generated using reverse-genetics as previously described using plasmids derived from pHW181-PB2, pHW182-PB1, pHW183-PA, pHW184-HA, pHW185-NP, pHW186-NA, pHW187-M, and pHW188-NS~\citep{hoffmann2000dna}.
To generate viruses from these plasmids, or variants described below, we transfected an equimolar mix of all eight plasmids into cocultures of 293T and MDCK-SIAT1 cells seeded at a ratio of 8:1. At 24 hr post-transfection, we changed media from D10 to influenza growth media (Opti-MEM supplemented with 0.01\% heat-inactivated FBS, 0.3\% BSA, 100 U of penicillin/ml, 100 $\mu$g of streptomycin/ml, and 100 $\mu$g of calcium chloride/ml). At 48-72h post-transfection we harvested the virus-containing supernatant, pelletted cellular material by centrifugation at 300 x g's for 4 min, and stored aliquots of the clarified viral supernatant at -80$^{\circ}$C.
We then titered thawed aliquots of viral by TCID50 on MDCK-SIAT1 cells, computing titers via the formula of Reed and Muench as described above.

For comparisons of interferon stimulation a relatively pure stock of WSN influenza was derived by harvesting reverse-genetics supernatants at 48h post infection and passaging virus on MDCK-SIAT1 cells for 36h at an MOI of 0.05---conditions which we previously validated to lead to stocks relatively depleted in defective particles~\citep{russell2018extreme}.

To generate an NS1-inactivated strain of influenza, six nucleotide changes resulting in the addition of five in-frame stop codons starting 10 nucleotides downstream of the 5' splice donor site, interrupting translation of NS1 while leaving NS2 (NEP) intact.
The viral RNA produced by this vector is described in (sequence)\abrcomment{I have uploaded the genbank files, just waiting for appropriate figure to append}.
This modified plasmid was used in a reverse-genetics approach to recapitulate mutant virus, harvesting transfection supernatant after 72h.
This virus was then passaged in MDCK-SIAT1 cells for 48h at an MOI of 0.05---due to slower growth kinetics than the wild-type virus this longer incubation was necessary.
Genome copy number of this virus relative to infectious titer was determined as described below by qPCR.

To generate viruses bearing internal deletions in the PB1 segment, indicated regions of the PB1 segment were excised by PCR from our reverse genetics plasmids.
Sequences are described in (sequence) \abrcomment{I have uploaded the genbank files and modified coordinates in map names to match mRNA coordinates, just waiting for appropriate figure to append}. 
To rescue these viruses, we used 293t cells and MDCK-SIAT1 cells transduced to constituitively express PB1 as previously described~\citep{Bloom2010oseltamivir}. 
Virus was harvested from transfections at 72h, and passaged twice in MDCK-SIAT1 cells expressing PB1 constituitively at an MOI of 0.001 for 72h and 0.01 for 48h. 
...
Viruses bearing potentially innate-immune stimulatory SNPs as identified in our single-cell analyses were recapitulated by first introducing these mutations to plasmids encoding the viral segments PB1 or NS by PCR. 
Sequences of these plasmids are described in (sequence). \abrcomment{I have uploaded the genbank files and modified coordinates in map names to match mRNA coordinates, just waiting for appropriate figure to append}. 
These viruses were rescued via reverse-transfection in a coculture of MDCK-SIAT1 and 293t cells alongside a matched wild-type control.
Each virus was rescued in two seperate transfection reactions. 
Transfection supernatants were harvested at either 72 hours post transfection (biological replicate \#2 of all SNPs tested) or 48-50h post infection (biological replicate \#1 of all SNPs tested and both replicates of wild-type influenza). 
Virus recovered from transfections was titered by hemagglutination, and MOI set to infectious dose as measured in the wild-type virus as measured by TCID50 on MDCK-SIAT1 cells. 
Particle-normalized viruses recovered from transfections were used directly in experiments in \abrcomment{Figure}. 


\subsection{flow cytometry analysis of interferon induction}
Our A549 lambda reporter cell line was seeded in D10 media 24h prior to infection.
At the time of infection, D10 media was replaced with influenza growth media and virus added to the indicated titers.
Cells were harvested 13hpi and stained for HA as described above.
...
After channels were compensated and cells gated to exclude multiplets and debris in FlowJo, data were analyzed using custom python pipelines described in \abrcomment{wherever we put those notebooks}.
Briefly, gating thresholds for interferon, mCherry, and HA staining were set to 99.9\% of an uninfected control.
