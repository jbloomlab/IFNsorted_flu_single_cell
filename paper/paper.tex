...
\FIGSUPP[IFNrare]{mice} shows that IFN+ cells were also rare in the single-cell mRNA-sequencing study of influenza-infected mice by \citet{steuerman2018dissection}.
...
The reporters were efficiently activated when infected with a strain of Sendai virus~\citep{strahle2006sendai} known to potently induce IFN (\FIGSUPP[IFNrare]{reporter_validation}).
...
{\bf (C)}
Frequency of IFN induction upon infection with the influenza virus stocks used in the single-cell studies in this paper quantified using the reporter cell line (see \FIGSUPP[IFNrare]{flow} for details).
...
\figsupp[Few cells express detectable IFN in the single-cell analysis of influenza-infected mice by \citet{steuerman2018dissection}.]
...
\figdata{textfile providing all primer sequences used to amplify and enrich influenza transcripts for PacBio sequencing. \url{https://github.com/jbloomlab/IFNsorted_flu_single_cell/tree/master/paper/figures/WorkflowSchematic/PacBio_primer_list.txt} 
}
...
Sequences of all primers described below are described in  \FIGDATA[experiment]{primer_sequences}.
All PCR amplifications were performed using Kapa HiFi Hotstart Readymix in a 100$\mu$l volume using each primer at a final concentration of 0.5$\mu$M and a 2 minute 15 second extension time and annealing temperature of 67$^{\circ}$C.
In order to enrich full-length cDNA derived from influenza transcripts, 1ng of full-length 10x-barcoded cDNA was used as template in a multiplex PCR reaction using eight segment-specific 5' primers and a shared, 10x-specific 3' primer.
To reduce strand-exchange and generate more even amplification across segments, this reaction was performed in an emulsion PCR using the Micellula DNA Emulsion Kit from Roboklon, supplementing our PCR reaction with additional BSA to a final concentration of 0.1mg/ml and an aqueous phase of 100$\mu$l .
Rather than the standard 98$^{\circ}$C melting step suggested by the manufacturer for Kapa HiFi we instead used a 95$^{\circ}$C melting step to prevent collapse of emulsion integrity.
...
This process physically separates disparate template molecules, preventing strand exchange, and, moreover, allowing each molecule to be amplified to exhaustion of its droplet's reagents without competing with the broader pool of PCR-amenable molecules~\citep{Boers:2015emulsion}.
...
As our depth on the three longest segments remained low (PB2, PB1, and PA), and those reads that we did possess were dominated by internally-deleted segments, we subjected 10ng of our amplified material for each segment to SPRI-bead selection using SPRIselect beads at a bead-PCR volume ratio of 0.4. 
This selection removes most low-molecular weight DNA species including internally-deleted defective segments.
Material from this selection was then amplified using 16 (PB1) or 14 (PB2 and PA) cycles of a non-emulsion PCR using standard conditions.
...
First, we first used 2.5ng of our already-amplified segment-specific material in a 10 cycle PCR to append circularization adapters, and cleaned the resultant mixture using SPRI-beads at a volume ratio of 0.4.
10ng of this amplified material was then used in a 20${\mu}$l NEBuilder reaction using an extended reaction time of 50 minutes in order to circularize our target segments using a Gibson reaction.
We then further incubated these reactions a  1 hour at 37$^{\circ}$C with exonuclease V and additional ATP to a final increase in concentration of 1mM.
As exonuclease V would be unable to act on closed circles, this step serves  to remove linear concatemers and uncircularized material.
Circularized and digested material was then bead-cleaned using SPRIselect beads at a volume ratio of 0.4.
This circularized material was then used in three non-emulsion PCRs specific to PB2, PB1, and PA, using two segment-specific primers that align to the central portion of each gene but in apposition to one-another at 20 (PB2), or 26 (PB1 and PA) cycles and bead-cleaned using a ratio of 1.0 volumes .
...
The cDNA was generated using SuperScript III first-strand synthesis supermix using the manufacturerâ€'s protocol, and using the universal vRNA primers of Hoffmann et al. (add Hoffman ref) with the modifications described in Xue et al.\citep{Xue:2017dla}. 
