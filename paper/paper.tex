...
We next set out to develop a method to determine whether features of the infecting virions are responsible for the heterogeneous outcome of influenza infection in single cells, including the rare induction of IFN.
Standard single-cell transcriptomic approaches are inadequate to for this purpose, since they typically just use 3'-end sequencing of poly-adenylated mRNAs to determine the abundance of each transcript~\citep{klein2015droplet, macosko2015highly, zheng2017massively, cao2017comprehensive, gierahn2017seq}.
These transcriptomic measurements of abundance do not reveal if the virions that infect individual cells have features (e.g., mutations) that contribute to heterogeneity in infection outcome. 
...
As indicated by the percentages shown for the original and MACS-sorted population, this process led to substantial enrichment for IFN+ cells.
...
We therefore developed the experimental approach in \FIG{workflow} to simultaneously determine the entire transcriptome and the full sequences of the infecting virions in single cells.
...
We have previously used synonymous barcoding near the 3' end of viral mRNAs to quantify the co-infection rate in single-cell transcriptomic data~\citep[about half of cells infected by multiple virions express a mix of wildtype and synonymously barcoded viral transcripts;][]{russell2018extreme}. 
...
We next used this viral stock to infect A549 cells carrying a type I IFN reporter (\FIG{transcriptomics}A) at a dose that we expected to lead to transcriptionally active viral infection in slightly less than half the cells.
...
In order to ensure the presence of IFN- cells, after the MACS enrichment we added back non-sorted cells to $\sim$10\% of the total.
...
We then processed the cells on a commercially available platform~\citep[the 10X Chromium;][]{zheng2017massively} that physically isolates cells in droplets and performs reverse-transcription of poly-adenylated mRNAs that appends a unique cell barcode to all cDNAs originating from a particular droplet, and a unique molecular identifier (UMI) to each cDNA molecules (\FIG{workflow}B).
...
Furthermore, because virtually all the influenza genome is transcribed into mRNA, the cell-barcoded transcripts cover nearly the entire viral genome: the only portions that are not captured in the cell-barcoded transcripts are one highly conserved nucleotide upstream of the transcription start site at nucleotide 2~\citep{koppstein2015sequencing}
