...
We generated a stock of A/WSN/1933 (H1N1) influenza (hereafter referred to as ``WSN''), and found that it activated the IFN reporter in $\sim$0.5\% of infected cells  (\FIG{IFNrare}B), a frequency roughly comparable to that reported in prior studies~\cite{killip2017single, russell2018extreme}.
...
These results are consistent with the established knowledge that IFN is expressed only in cells that directly detect infection, but that ISGs are also expressed via paracrine signaling in other cells~\cite{stetson2006type,honda2006type}.
...
Finally, prior work has suggested that virions with internal deletions in the polymerase genes can induce higher levels of IFN~\cite{wang2018cell, baum2010preference, tapia2013defective, boergeling2015evidence, dimmock2015cloned}.
...
Some of the viral defects in IFN+ are easy to reconcile with existing knowledge: for instance, NS1 is the virus's primary IFN antagonist~\cite{garcia1998influenza, hale2008multifunctional}, and internal deletions are known to be prevalent in immunostimulatory viral stocks~\cite{wang2018cell, baum2010preference, tapia2013defective, boergeling2015evidence, dimmock2015cloned}.
Other defects are more surprising: for instance, it is not obvious why amino-acid mutations in PB1 increase IFN induction.
We therefore designed experiments to interrogate the mechanisms of some of these defects.

We first focused on the PB1 mutations.
Recent research has shown that mutations to the PB2 polymerase subunit of the WSN viral strain can enhance IFN induction by leading to the production of aberrant mini viral RNAs (mvRNAs) that trigger RIG-I~\cite{velthuis2018mini}.
To investigate if the PB1 mutations act by a similar mechanism, we first examined their location in a structural model of the polymerase complex (\FIG{PB1}A).
The IFN-enhancing PB1 mutation T677A occurs at the tip of a helix that interacts with the 3' terminus of the RNA template as it enters the channel above the PB1 active site, whereas the IFN-enhancing D27N mutation is deeper in the polymerase and close to the binding pocket of the 5' terminus of the template. 
Therefore, both mutations could plausibly affect the polymerase's activity by changing its interactions with the RNA template.
...
\caption{
IFN-inducing mutations D27N and T677A in the PB1 protein affect polymerase activity \textit{in vitro}. 
(A) Model of bat influenza A virus RNA polymerase (PDB 4WSB)~\cite{reich2014structural} superposed with the influenza B virus RNA polymerase (PDB 5MSG)~\cite{reich2017vitro}.
...
(B) Polymerase activity on full-length NA vRNA template in 293T cells.
...
Other panels show Western blot analysis of PB1, NP and GAPDH protein expression. 
Graph shows the average of three biological repeats. 
(C) Panel shows PAGE analysis of RT-PCR of mvRNAs.
The same amount of total input RNA was used for each sample.
The length of the mmvRNAs was extended by the adapters (adapt) present in the primers.
Graph shows the IFN-beta promoter activity in transfected 293T cells as measured using a dual luciferase reporter assay.
...
The other two panels show the PB1 and tubulin expression levels analyzed by western blot. 
The top graph shows the average of three biological repeats. 
...
{\color{red} Legend will be further edited and figure possibly re-arranged after all data are added.}
}
\label{fig:PB1}
...
To test if the PB1 mutations affect polymerase activity on a full-length viral RNA template, we expressed wild-type or mutant PB1 protein along with the other proteins in the polymerase complex (PB2, PA, and NP) and the viral RNA (vRNA) for the NA segment in 293T cells.
Twenty-four hours after transfection, we extracted total RNA and analyzed replication (vRNA level) and transcription (mRNA level) activity by primer extension.
Both mutations had activities that were significantly different from wild type, despite being expressed at wild-type protein levels (\FIG{PB1}B).
Specifically, T677A had higher levels of both activities, whereas D27N had reduced levels of both activities---although D27N still retained activity far in excess of a control active-site mutant (\FIG{PB1}B).
To test if these activity changes increased production of immunostimulatory mvRNAs~\cite{velthuis2018mini}, we fractionated the total RNA to enrich for small species (17-200 nt long) and used reverse transcription followed by PCR (RT-PCR) to amplify species with both the 3' and 5' vRNA termini. 
The D27N mutant produced more mvRNAs than wild type, while the T667A mutant produced the same low levels of mvRNAs as wild type (\FIG{PB1}C).
{\color{red} Analysis of the IFN levels in these cells showed that... \textit{(Still need to figure out how to work this in after experiments done.)} }

Because the D27N PB1 mutant might produced more mvRNAs and had reduced activity on a full-length vRNA template, we speculated that it might have reduced polymerase processivity.
To confirm this, we repeated the \textit{in vitro} polymerase activity assays using a short 246-nucleotide template~\cite{velthuis2018mini} in place of the full-length NA vRNA (\FIG{PB1}D).
On this shorter template, the activity of the D27N mutant was similar to wild type, while the activity of the T677A mutant remained higher than wildtype (although not significantly higher in three biological repeats). 
{\color{red} Analysis of the IFN expression level in response to viral replication on the short template showed higher IFN induction by both PB1 mutants. \textit{(Need to work in prior sentence and add a wrap up sentence; will wait to do this until we have all the IFN results. End with something about how even PB1 point mutants act by different mechanisms (more activity versus more mvRNAs) to tie into next paragraph.)}}

%%%%% begin IFN_vs_HA figure %%%%%%%%
\begin{figure}
\centerline{\includegraphics[width=0.35\textwidth, valign=t]{figures/MutantComparison/p_foldchange.pdf}}
\caption{
Cells that express higher levels of HA protein are much more likely to induce IFN expression only if they are infected by virus with defects in NS1.
The y-axis shows the ratio of the percent of IFN+ cells in the highest HA-expression quartile relative to the lowest HA-expression quartile among infected cells as determined by flow cytometry.
Points indicate replicates, and lines indicate the mean.
This figure is based on joint analysis of the IFN reporter and HA staining for all infected cells in the flow cytometry data in \SUPPFIG{validationflow}; see \SUPPFIG{IFN_vs_HA} for a more detailed breakdown of the frequency of IFN induction as a function of HA expression.
}
\label{fig:IFN_vs_HA}
\end{figure}
%%%%% end IFN_vs_HA figure %%%%%%%%

The diversity of mechanisms of IFN induction extends to other types of viral defects.
For instance, the single-cell data indicates that increased viral gene expression is only associated with increased IFN induction when the infecting virion fails to express NS (\FIG{mutations}D).
To confirm this observation by an independent method, we performed a detailed analysis of the flow cytometry underlying \FIG{validation} to determine relationship between IFN induction and expression of the viral HA protein.
\FIG{IFN_vs_HA} shows that cells that express more HA are much more likely to turn IFN+ if they are infected with the NS1stop or NS1-A122V mutants, but not if they are infected with any of the other viral variants. 
Taken together with the single-cell data, this observation suggests that only in the absence of functional NS1 does higher viral gene expression predispose an infected cell to induce IFN.
Therefore, the diverse range of viral defects that induce IFN act by diverse processes, showing that viral variation influences not only the rate of IFN induction but also the factors that contribute to this induction.


\section{Discussion}
We have determined the full sequences of all viral genes in single influenza-infected cells.
Methodologically, our major advance is to measure the \emph{genotypes} of viruses in addition to the abundance of viral components (i.e., transcripts, proteins, or progeny virions) as has been done by prior single-cell studies~\cite{russell2018extreme, steuerman2018dissection, heldt2015single, wang2018cell, brooke2013most, zanini2018single, zanini2018virus, saikia2018simultaneous, oneal2018west, zhu2009growth, schulte2014single, akpinar2016high}.
...
We speculate that these alterations increase production of aberrant immunostimulatory products such as mini-viral RNAs~\cite{velthuis2018mini} or other deleted forms of vRNAs.
Finally, we found an internal deletion in PB1 that enhances IFN induction, consistent with prior work showing such deletions are immunostimulatory~\cite{baum2010preference, tapia2013defective, boergeling2015evidence, dimmock2015cloned}.
...
Additionally, the relative importance of different defects likely varies across infection conditions, viral strains, and cell types: it remains an open question which defects are most relevant for immune detection during actual human infections.
...
Our results further show that viral genetic defects only partially explain the heterogeneity among influenza-infected cells.
...
Additionally, no viral defect induces IFN deterministically: every type of immunostimulatory defect we characterize is also observed in IFN- cells in our single-cell dataset.
This fact shows that stochasticity or pre-existing cellular states play a major role in affecting the outcome of infection---a finding that concords with the fact that IFN induction is heterogeneous even among cells treated with synthetic innate-immune ligands~\cite{shalek2013single, shalek2014single, wimmers2018single, bhushal2017cell}, as well as for other viruses~\cite{oneal2018west}.
...
It is therefore plausible that early heterogeneity could affect downstream events.
