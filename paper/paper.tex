...
Because influenza has a high mutation rate, even if the \emph{consensus} sequence of a viral population is ``wildtype,'' individual virions will often have defects~\citep{parvin1986measurement, suarez1992heterogeneity, suarez1994estimation, bloom2014experimentally, pauly2017novel}.
...
None of these techniques reliably reveal if the virion infecting a specific cell has some idiosyncratic mutation, despite the fact that such mutations are common regardless of whether the cells are infected with a nominally ``wildtype'' or ``mutant'' stock of virus.
...
Even though we used a nominally ``wildtype'' stock of virus, two-thirds of cells are infected by virions that have a mutation or defect in gene expression.
...
However, viral genetic variation does not fully explain the cell-to-cell heterogeneity, as even truly wildtype virions sometimes induce IFN, and no viral mutations deterministically induce IFN.
...
A challenge in studying IFN induction by influenza virus is that infection only rarely triggers expression of anti-viral IFN~\citep[type I and type III;][]{kotenko2017contribution} at the single-cell level~\citep{killip2017single}.
...
Simple visual inspection of \FIG{genotypes} makes it glaringly obvious how any experimental approach that does not directly sequence the viral genes in single cells is really averaging across a vast number of hidden viral defects.

It turns out that these idiosyncratic viral defects explain much of the heterogeneity in viral transcriptional burden that has been observed in prior single-cell studies of influenza~\citep{russell2018extreme, steuerman2018dissection, heldt2015single, sjaastad2018distinct}.
If we examine only cells infected by truly wildtype virions, we see that the heterogeneity in viral transcriptional burden is substantially reduced (\FIG{mutations}A).

Crucially, viral defects also play an important role in determining if infected cells trigger an innate-immune response.
We identified four types of viral defects in single IFN+ cells that we validated to increase IFN induction. 
Two types of defects---absence of the NS gene and amino-acid mutation to the NS1 protein---likely act by impairing NS1's well-known ability to antagonize innate-immune pathways~\citep{garcia1998influenza, hale2008multifunctional}.
...
In fact, given the extensive prior work describing such deletions, it is perhaps surprising that we did not identify more of them in our single IFN+ cells.
There may be several reasons: first, we used relatively pure viral stocks~\citep{xue2016propagation} at modest MOI~\citep[conditions that better reflect the early stages of real human influenza infections;][]{mccrone2018stochastic, xue2018reconciling, varble2014influenza}; second, our experiments preferentially captured cells with higher viral transcriptional load; and third, most prior studies have used techniques that can detect large deletions but not subtle point mutations.
But we emphasize that the relative importance of different types of viral defects for IFN induction likely varies across infection conditions, viral strains, and cell types---so it certainly remains an open question as to which defects are most relevant for early immune detection of actual viral infections in humans.
