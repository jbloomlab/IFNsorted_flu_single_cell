...
\subsection{A system to identify and enrich for IFN+ positive cells}
...
For instance, we previously performed single-cell mRNA sequencing of hundreds of influenza-infected A549 cells at early times (6 to 10 hours) post-infection~\citep{russell2018extreme}.
Only one of these cells expressed detectable type I or type III IFN (\FIG{IFNrare}A).
Importantly, the paucity of IFN+ cells identified in our prior single-cell mRNA sequencing was not due simply to a poor detection threshold, since $>$100 IFN transcripts were detected in the one IFN+ cell~\citep{russell2018extreme}.
...
In further support of these findings, we re-analyzed the single-cell mRNA sequencing of influenza-infected mice by \citet{steuerman2018dissection}.
As shown in \FIGSUPP[IFNrare]{mice}, many infected cells in the mice expressed interferon-stimulated genes, which are induced autocrine and paracrine signaling~\citep{stetson2006type,honda2006type}.
...
{\bf (A)} Number of influenza-infected and IFN+ cells in our prior single-cell study of influenza infection of A549 cells~\citep{russell2018extreme}.
\FIGSUPP[IFNrare]{mice} shows that IFN+ cells were also rare in the single-cell mRNA-sequencing of influenza-infected mice by \citet{steuerman2018dissection}.
...
\figsupp[Few cells express detectable IFN in the single-cell mRNA-sequencing of influenza-infected mice by \citet{steuerman2018dissection}.]
...
In order to efficiently identify and enrich cells that express IFN, we integrated IFN reporters into A549 cells (\FIG{IFNrare}B).
These reporters consisted of a type I (IFNB1) or type III (IFNL1) promoter driving expression of a cell-surface protein~\citep[LNGFR$\Delta$C;][]{bonini1997hsv,ruggieri1997cell} followed by a fluorescent protein.
...
We validated that these A549 reporter cell lines were efficiently activated by infection with a strain of Sendai virus~\citep{strahle2006sendai} that potently induces IFN (\FIGSUPP[IFNrare]{reporter_validation}).
We also used the reporters to show that expression of type I and type III IFN is highly correlated in single influenza-infected A549 cells (\FIGSUPP[IFNrare]{type_I_vs_III}), a result that is further validated by the single-cell mRNA sequencing described below.
...
We then used one of the reporter cell lines to quantify the frequency of IFN induction in single A549 cells infected with the A/WSN/1933 (H1N1) strain of influenza.
As shown in \FIG{IFNrare}C, $\sim$0.5\% of infected cells activated the IFN reporter at 13 hours post-infection.
This frequency of IFN induction was roughly comparable to what we had observed in our prior single-cell mRNA sequencing of influenza-infected cells (\FIG{IFNrare}B), with the slightly higher rate in \FIG{IFNrare}C probably due to using a different viral stock and slightly later timepoint.
...
\hspace{0.04in} {\bf A} \includegraphics[width=0.95\linewidth, valign=t]{figures/WorkflowSchematic/SchematicForPaper.pdf}
\vspace{0.1in}

\includegraphics[width=\linewidth, clip=false]{figures/single_cell_figures/p_cell_summary.pdf}

\caption{
Single-cell sequencing of influenza infections that induce IFN.
{\bf (A)} Experimental workflow.
(1) IFN reporter A549 cells are infected with a mix of wildtype and synonymously barcoded viruses.
IFN+ cells are enriched by MACS (\FIGSUPP[experiment]{MACS}), and pooled with non-enriched cells and uninfected canine cells that serve as an internal control for multiplets and mRNA leakage.
...
{\bf (H)} Fraction of cellular mRNA derived from IFN across all cells, faceted by whether the cells are infected.
...
Many cells that do not express IFN itself still express ISGs (\FIGSUPP[experiment]{ISG}).
}
...
The A549 cells with the IFN-$\beta$ LNGFR$\Delta$C-mNeonGreen reporter were infected with wildtype A/WSN/1933 influenza (two different viral stocks) at a target MOI of 0.1 TCID50 per cell.
...
The sequences of all gene segments from the wildtype and synonymously barcoded viral strains are in \FIGDATA[experiment]{virus_seqs}.
...
A schematic of the experiment is shown in \FIG{experiment}A.
...
The cells were then infected with this mixture at a dose designed to infect about half the cells (\FIG{experiment}D suggests that the actual rate of detectable infection was slightly lower).
...
This MACS sorting is expected to give approximately the enrichment for IFN+ cells shown in \FIGSUPP[experiment]{MACS}.
...
At this point, uninfected canine (MDCK-SIAT1) cells were also added to approximately 10\% of the final cell fraction to enable quantification of the cell multiplet rate (\FIG{experiment}B) and background viral mRNA in uninfected cells (\FIG{experiment}D). 
...
The number of sequences obtained for each gene after pooling the data from all reactions is shown in \FIGSUPP[experiment]{CCSs}, which also indicates that the net rate of strand exchange is very low (see \FIGSUPP[experiment]{StrandExchange} for an illustration of how this is determined).
