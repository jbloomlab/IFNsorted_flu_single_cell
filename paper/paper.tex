...
The reporters were efficiently activated by an IFN-inducing strain of Sendai virus (\SUPPFIG{reporter_validation}).
{\bf (B)}
Frequency of IFN induction upon infection with the influenza virus stock used in the single-cell studies in this paper, as quantified using the type III IFN reporter cell line (see \SUPPFIG{flow} for details).
...
The reporters were efficiently activated by a strain of Sendai virus~\citep{strahle2006sendai} that potently induces IFN (\SUPPFIG{reporter_validation}), and activation of the type I and type III IFN reporters was highly correlated in our cells (\SUPPFIG{type_I_vs_III}; further validated by the single-cell transcriptomics below).
...
{\bf (A)}
Percent of infected cells that become IFN+ after infection with a bulk stock of the indicated viral mutant, as determined using a reporter cell line (see \SUPPFIG{SNPflow} for details).
...
See \SUPPFIG{delflow} and \SUPPFIG{delqPCR} for more detailed experimental data.
...
For the dual type I / type III reporter used in \SUPPFIG{type_I_vs_III}, a single-cell clone of the type III reporter cell line was transduced with the type I reporter bearing the mCherry fluorescent marker, and then isolated and propagated as a single cell clone for the other cell lines.
All reporter lines tested negative for mycoplasma contamination by the Fred Hutch Genomics Core.

\SUPPFIG{reporter_validation} shows validation of the reporter cell lines using infection with saturating amounts of the Cantell strain of Sendai virus (obtained from Charles River Laboratories).
...
At 50 hours post-transfection (for the replicate 1 viruses in \SUPPFIG{SNPflow}) or 72 hours (for the replicate 2 viruses in \SUPPFIG{SNPflow}), we harvested the virus-containing supernatant, clarified this supernatant by centrifugation at 300$\times$g for 4 min, and stored aliquots of the clarified viral supernatant at -80$^{\circ}$C.
We then thawed aliquots and titered by TCID50 on MDCK-SIAT1 cells.
For the infections in \SUPPFIG{SNPflow}, we wanted to use equivalent particle counts, so we normalized all viruses to an equivalent hemagglutination titer on turkey red blood cells~\citep{hirst1942quantitative}.
...
We then performed infections of the A549 reporter cell line at equivalent hemagglutination titer and analyzed the data as described in \SUPPFIG{SNPflow}. 

To generate the NS1stop mutant virus and the wild-type and PB1del385to2163 mutant viruses in \SUPPFIG{delflow}, we used slightly different procedures.
...
The infections in \SUPPFIG{delflow} were performed at equivalent TCID50s as described in the legend to that figure.
...
\caption{Flow cytometry data for \FIG{IFNrare}B.
...
\caption{Flow cytometry data for \FIG{validation}A.
...
Therefore, for the data in this figure all cells were infected at an equivalent MOI of 0.3 as determined by TCID50 on MDCK-SIAT1 cells for the wild-type and NS1stop viruses, and on MDCK-SIAT1 cells expressing PB1~\citep{bloom2010permissive} for the PB1del385to2163 virus.
Finally, the approach in \SUPPFIG{delqPCR} was used to show that equivalent TCID50s, all viral variants had similar amounts of transcriptionally active virus in the absence of secondary transcription.
...
\label{suppfig:delflow}
\end{suppfig}

\begin{suppfig}
\includegraphics[width=0.6\textwidth]{figures/Validation_Figure/qpcr_plot.pdf}
\caption{
Validation that the infections in \FIG{validation}B and \SUPPFIG{delflow} were performed at similar doses of virions capable of initiating primary transcription.
In this experiment, A549 cells were infected at MOI of 0.4 (based on TCID50 as described in \SUPPFIG{delflow}), and then after 8 hours mRNA was harvested for qPCR on oligo-dT primed reverse transcription products.
