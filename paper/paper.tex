...
A challenge in studying the induction of IFN by influenza is that infection with this virus only rarely triggers IFN expression at the single-cell level~\citep{killip2017single}.
...
Importantly, the paucity of IFN+ cells identified in our prior single-cell transcriptomics was not due simply to a poor detection threshold, since $>$100 IFN transcripts were detected in the one IFN+ cell~\citep{russell2018extreme}.

The rareness of IFN induction by influenza infection is not merely a feature of A549 cells in culture.
Prior studies using fluorescent reporters have found that IFN expression is very rare at the single-cell level in influenza-infected mice at 24 hours post-infection~\citep{kallfass2013visualizing}.
...
As shown in \FIGSUPP[IFNrare]{mice}, many infected cells in the mice expressed IFN-stimulated genes, which are induced autocrine and paracrine signaling~\citep{stetson2006type,honda2006type}.
...
{\bf (A)} Number of infected and IFN+ cells in our prior single-cell transcriptomics of influenza-infected A549 cells~\citep{russell2018extreme}.
...
{\bf (B)} An A549 reporter cell line to identify single cells expressing IFN.
...
We created reporter cell lines with type I (IFN-$\beta$) and type III (IFN-$\lambda$) promoters (\FIGDATA[IFNrare]{reporter_sequences}).
The reporters were efficiently activated when infected with an IFN-inducing strain of Sendai virus (\FIGSUPP[IFNrare]{reporter_validation}).
...
\figsupp[Few cells express detectable IFN in the single-cell transcriptomics of influenza-infected mice by \citet{steuerman2018dissection}.]
...
However, only 5 of 1220 cells express any detectable type I (IFN-$\alpha$ and IFN-$\beta$) or type III (IFN-$\lambda$) mRNAs, and only at low levels (one mRNA transcript detected).
...
At 13 hours post-infection, activation of the IFN reporter was then monitored by flow cytometry using the marker indicated at the bottom of each plot (either a fluorescent protein or antibody staining for the cell-surface LNGFR using a PE-conjugated anti-LNGFR antibody from Miltenyi Biotec).
...
The A549 cells with the IFN-$\lambda$ reporter driving LNGFR$\Delta$C-ZsGreen were not infected, infected with saturating amounts of the Cantell strain of Sendai virus~\citep{strahle2006sendai}, or infected the same stock of influenza virus used in the single-cell experiment at a target MOI of 0.3.
...
In order to efficiently identify and enrich cells that express IFN, we integrated IFN reporters into the genome of A549 cells (\FIG{IFNrare}B).
...
Cells that activate IFN will express the cell-surface protein, enabling them to be enriched by magnetic-activated cell sorting (MACS).
The frequency of IFN+ cells can also be determined by flow cytometry for the fluorescent protein.
We validated that the IFN reporters were efficiently activated by infection with a strain of Sendai virus~\citep{strahle2006sendai} that potently induces IFN (\FIGSUPP[IFNrare]{reporter_validation}).
We also used the reporters to show that expression of type I and type III IFN is highly correlated in single influenza-infected A549 cells (\FIGSUPP[IFNrare]{type_I_vs_III}), a result that is further validated by our single-cell transcriptomics described below.
...
We then used the reporter cells to quantify the frequency of IFN induction in single A549 cells infected with the A/WSN/1933 (H1N1) strain of influenza (hereafter referred to as ``WSN'').
As shown in \FIG{IFNrare}C, $\sim$0.5\% of infected cells activated the IFN reporter at 13 hours post-infection.
This frequency of IFN induction was roughly comparable to what we had observed in our prior single-cell transcriptomics of influenza-infected cells (\FIG{IFNrare}B), with the slightly higher rate in \FIG{IFNrare}C probably due to using a different viral stock and slightly later timepoint.

\subsection{Simultaneous transcriptomics and virus-sequencing of single infected cells}
We next set out to determine whether features of the infecting virions are responsible for the heterogeneous outcome of influenza infection in single cells, including the rare induction of IFN.
Standard single-cell transcriptomic approaches are inadequate to address this question, since they typically just use 3'-end sequencing of poly-adenylated mRNAs to determine the abundance of each transcript~\citep{klein2015droplet, macosko2015highly, zheng2017massively, cao2017comprehensive, gierahn2017seq}.
But simply measuring the abundance of viral transcripts is insufficient for our purposes: we want to know the full sequences of all viral genes in each infected cell, so that we can determine if features of these genes (e.g., mutations) contribute to heterogeneity in infection outcome. 
...
{\bf (B)}
The mRNAs from individual cells are tagged with cell-specific barcodes using a 10X Chromium device.
{\bf (C)}
...
The A549 cells with the IFN-$\beta$ LNGFR$\Delta$C-mNeonGreen reporter were infected with wildtype WSN influenza (two different viral stocks) at a target MOI of 0.1 TCID50 per cell.
...
As indicated by the percentages shown for the original and MACS-sorted population, this process led to between 50- and 350-fold enrichment for IFN+ cells.
...
We developed the experimental approach in \FIG{workflow} to simultaneously determine the entire transcriptome and the full sequences of the infecting virions in single cells.
First, we generated a stock of virus that consisted of a mix of two variants of the WSN strain: the wildtype virus and a ``synonymously barcoded'' variant that contains two engineered synonymous mutations near each end of each gene (\FIGDATA[workflow]{virus_seqs}).
We have previously shown that synonymously barcoding viral genes near the 3' end of the mRNA enables quantification of the co-infection rate in single-cell transcriptomic data, since about half the cells infected by multiple virions express a mix of wildtype and synonymously barcoded viral transcripts~\citep{russell2018extreme}. 
New to this study, we also added synonymous barcodes near the 5' end of the mRNA; as explained below, having barcodes at both ends provides an important control for PCR artifacts during full-length sequencing of viral transcripts.

We next used this viral stock to infect A549 cells carrying a type I IFN reporter (\FIG{transcriptomics}A), using a viral dose that we expected to lead to transcriptionally active viral infection in slightly less than half the cells.
From 12 to 13 hours post-infection, we used magnetic columns to enrich for cells that expressed the cell-surface protein driven by the IFN reporter.
Based on pilot experiments, we expected this MACS to enrich the rare IFN+ cells to about 5\% to 30\% of the total population (\FIGSUPP[workflow]{MACS}; the MACS enrichment efficiency is imperfect so many sorted cells are still IFN-).
In order to ensure the presence of IFN- cell, we also added back non-sorted cells to $\sim$10\% of the total.
We also added uninfected canine cells to $\sim$5\% of the total to serve as an internal control for multiplets and to estimate the background amount of viral mRNA observed in the transcriptomes of truly uninfected cells.
...
To identify cells that were co-infected with both viral barcodes (\FIG{transcriptomics}F), we used a binomial test to identify cells for which we could reject the null hypothesis that at least 95\% of viral mRNA was derived form the more common viral barcode.
