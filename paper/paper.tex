...
Four of these viruses contained point mutations in PB1, two contained point mutations in NS, and one contained an internal deletion in PB1.
In addition, we created a virus with an inactivated NS1 to mimic the infections that failed to express NS (it was not possible to generate a viral stock that completely lacked the NS segment).
...
For the infections in \FIGSUPP[validation]{SNPflow}, we wanted to use equivalent particle counts, so we normalized all viruses to an equivalent HI titer on \jdbcomment{red blood cells from what species where, and is there citation to protocol?}.
We then performed infections of the A549 reporter cell line at equivalent HI titer and analyzed the data as described in \FIGSUPP[validation]{SNPflow}. 
